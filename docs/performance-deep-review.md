...
日期：2026-05-28
状态：current（A1/A2 已落地；B/C 类保留为后续执行建议）
...
A1、A2 已完成。下一步如继续优化，先做 B1→B2→B3（核心级联，需测试守门）→ C 类一律等用户指令 + profiling。B 类是消除主级联的关键，但因为正确性敏感，建议人工复核而非完全盲执行。
